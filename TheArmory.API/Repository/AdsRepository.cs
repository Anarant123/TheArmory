...
public class AdsRepository : BaseRepository
{
    ...
    AdLocationCommand command)
  {
    ...
    if (double.TryParse(latitudeStr.Replace('.', ','), out var latitude))
    {
      location.Latitude = latitude;
    }
    else
    {
      return new BaseResult<MyAdViewModel>("Неверный формат широты");
    }

    if (double.TryParse(longitudeStr.Replace('.', ','), out var longitude))
    {
      location.Longitude = longitude;
    }
    else
    {
      return new BaseResult<MyAdViewModel>("Неверный формат долготы");
    }
    ...
    {
      ad.Location.Address = location.Address;
      ad.Location.Latitude = location.Latitude;
      ad.Location.Longitude = location.Longitude;
    }
